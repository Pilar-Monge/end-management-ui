...
const SurvivorCharacter = memo(
  forwardRef<SurvivorCharacterRef, SurvivorCharacterProps>(function SurvivorCharacter(
    ...
    useEffect(() => {
      ...
      const skinMaterial = new THREE.MeshStandardMaterial({ color: COLORS.skin, roughness: 0.75, metalness: 0 });
      const hairMaterial = new THREE.MeshStandardMaterial({ color: COLORS.hair, roughness: 0.6, metalness: 0.05 });
      const beardMaterial = new THREE.MeshStandardMaterial({ color: COLORS.beard, roughness: 0.85, metalness: 0.02 });
      const browMaterial = new THREE.MeshStandardMaterial({ color: '#b07858', roughness: 0.75, metalness: 0 });
      const eyebrowMaterial = new THREE.MeshStandardMaterial({ color: COLORS.eyebrows, roughness: 0.8, metalness: 0 });
      const shirtMaterial = new THREE.MeshStandardMaterial({ color: COLORS.shirt, roughness: 0.72, metalness: 0.04 });
      const vestMaterial = new THREE.MeshStandardMaterial({ color: COLORS.innerVest, roughness: 0.7, metalness: 0.03 });
      const vestPocketMaterial = new THREE.MeshStandardMaterial({ color: '#5a6830', roughness: 0.75, metalness: 0.02 });
      const jacketMaterial = new THREE.MeshStandardMaterial({ color: COLORS.outerJacket, roughness: 0.65, metalness: 0.06 });
      ...
      const pantsMaterial = new THREE.MeshStandardMaterial({ color: COLORS.pants, roughness: 0.8, metalness: 0.04 });
      const thighPocketMaterial = new THREE.MeshStandardMaterial({ color: COLORS.thighPocket, roughness: 0.82, metalness: 0.03 });
      const kneeMaterial = new THREE.MeshStandardMaterial({ color: '#4a3820', roughness: 0.86, metalness: 0.02 });
      const beltMaterial = new THREE.MeshStandardMaterial({ color: COLORS.belt, roughness: 0.82, metalness: 0.04 });
      const buckleMaterial = new THREE.MeshStandardMaterial({ color: COLORS.beltBuckle, roughness: 0.5, metalness: 0.5 });
      const bootsMaterial = new THREE.MeshStandardMaterial({ color: COLORS.boots, roughness: 0.85, metalness: 0.03 });
      const bootSoleMaterial = new THREE.MeshStandardMaterial({ color: COLORS.bootSole, roughness: 0.92, metalness: 0.02 });
      const pedestalMaterial = new THREE.MeshStandardMaterial({ color: COLORS.pedestal, roughness: 0.55, metalness: 0.35 });
      const pedestalSheenMaterial = new THREE.MeshStandardMaterial({ color: COLORS.pedestalSheen, roughness: 0.42, metalness: 0.48 });

      const pedestal = new THREE.Mesh(new THREE.CylinderGeometry(0.75, 0.85, 0.14, 48), pedestalMaterial);
      pedestal.position.y = -0.07;
      applyShadow(pedestal);
      characterGroup.add(pedestal);

      const pedestalSheen = new THREE.Mesh(new THREE.CylinderGeometry(0.72, 0.74, 0.02, 48), pedestalSheenMaterial);
      ...
      const leftUpperLeg = new THREE.Mesh(new THREE.CylinderGeometry(0.145, 0.135, 0.58, 14), pantsMaterial);
      leftUpperLeg.position.set(-0.18, 1.05, 0);
      leftUpperLeg.rotation.z = 0.04;
      applyShadow(leftUpperLeg);
      characterGroup.add(leftUpperLeg);

      const rightUpperLeg = new THREE.Mesh(new THREE.CylinderGeometry(0.145, 0.135, 0.58, 14), pantsMaterial);
      ...
      const leftThighPocket = new THREE.Mesh(new THREE.BoxGeometry(0.13, 0.16, 0.06), thighPocketMaterial);
      leftThighPocket.position.set(-0.3, 1.08, 0.04);
      applyShadow(leftThighPocket);
      characterGroup.add(leftThighPocket);

      const rightThighPocket = new THREE.Mesh(new THREE.BoxGeometry(0.13, 0.16, 0.06), thighPocketMaterial);
      rightThighPocket.position.set(0.3, 1.08, 0.04);
      applyShadow(rightThighPocket);
      characterGroup.add(rightThighPocket);

      const leftPocketStrap = new THREE.Mesh(new THREE.BoxGeometry(0.13, 0.025, 0.07), thighPocketMaterial);
      leftPocketStrap.position.set(-0.3, 0.98, 0.04);
      applyShadow(leftPocketStrap);
      characterGroup.add(leftPocketStrap);

      const rightPocketStrap = new THREE.Mesh(new THREE.BoxGeometry(0.13, 0.025, 0.07), thighPocketMaterial);
      rightPocketStrap.position.set(0.3, 0.98, 0.04);
      applyShadow(rightPocketStrap);
      characterGroup.add(rightPocketStrap);

      const leftLowerLeg = new THREE.Mesh(new THREE.CylinderGeometry(0.125, 0.14, 0.55, 14), pantsMaterial);
      leftLowerLeg.position.set(-0.18, 0.5, 0);
      applyShadow(leftLowerLeg);
      characterGroup.add(leftLowerLeg);

      const rightLowerLeg = new THREE.Mesh(new THREE.CylinderGeometry(0.125, 0.14, 0.55, 14), pantsMaterial);
      ...
      const leftBootShaft = new THREE.Mesh(new THREE.CylinderGeometry(0.13, 0.125, 0.36, 14), bootsMaterial);
      leftBootShaft.position.set(-0.18, 0.18, 0);
      applyShadow(leftBootShaft);
      characterGroup.add(leftBootShaft);

      const rightBootShaft = new THREE.Mesh(new THREE.CylinderGeometry(0.13, 0.125, 0.36, 14), bootsMaterial);
      ...
      const jacketShell = new THREE.Mesh(new THREE.BoxGeometry(0.62, 0.7, 0.33), jacketShellMaterial);
      ...
      const leftHairFlow = new THREE.Mesh(new THREE.CylinderGeometry(0.1, 0.06, 0.7, 8), hairMaterial);
      ...
      const rightHairFlow = new THREE.Mesh(new THREE.CylinderGeometry(0.1, 0.06, 0.7, 8), hairMaterial);
      ...
      const leftUpperArm = new THREE.Mesh(new THREE.CylinderGeometry(0.105, 0.115, 0.5, 12), jacketMaterial);
      leftUpperArm.position.set(-0.42, 2.0, 0);
      leftUpperArm.rotation.z = 0.2;
      applyShadow(leftUpperArm);
      characterGroup.add(leftUpperArm);

      const rightUpperArm = new THREE.Mesh(new THREE.CylinderGeometry(0.105, 0.115, 0.5, 12), jacketMaterial);
      ...
      const leftCuff = new THREE.Mesh(new THREE.CylinderGeometry(0.105, 0.108, 0.06, 12), jacketMaterial);
      leftCuff.position.set(-0.47, 1.77, 0);
      leftCuff.rotation.z = 0.2;
      applyShadow(leftCuff);
      characterGroup.add(leftCuff);

      const rightCuff = new THREE.Mesh(new THREE.CylinderGeometry(0.105, 0.108, 0.06, 12), jacketMaterial);
      rightCuff.position.set(0.47, 1.77, 0);
      rightCuff.rotation.z = -0.2;
      applyShadow(rightCuff);
      characterGroup.add(rightCuff);

      const leftForearm = new THREE.Mesh(new THREE.CylinderGeometry(0.088, 0.1, 0.44, 12), skinMaterial);
      leftForearm.position.set(-0.5, 1.58, 0);
      leftForearm.rotation.z = 0.12;
      applyShadow(leftForearm);
      characterGroup.add(leftForearm);

      const rightForearm = new THREE.Mesh(new THREE.CylinderGeometry(0.088, 0.1, 0.44, 12), skinMaterial);
