...
const MODES: Record<string, ModeConfig> = {
  Hazy:   { skyColor: '#f0dcc5', fogColor: '#f0dcc5', fogDensity: 0.025, lightIntensity: 1.5,  lightColor: '#ffe0b3', ambientIntensity: 0.3 },
  Early:  { skyColor: '#ffd8da', fogColor: '#ffe0c0', fogDensity: 0.015, lightIntensity: 1.2,  lightColor: '#ffecd2', ambientIntensity: 0.3 },
  Summer: { skyColor: '#88bbdd', fogColor: '#99ccdd', fogDensity: 0.008, lightIntensity: 2.0,  lightColor: '#ffffcc', ambientIntensity: 0.4 },
  Noche:  { skyColor: '#1a1a3a', fogColor: '#202045', fogDensity: 0.01, lightIntensity: 0.8,  lightColor: '#e0e0ff', ambientIntensity: 0.25, showStars: true },
  Storm:  { skyColor: '#2c3e50', fogColor: '#34495e', fogDensity: 0.03, lightIntensity: 0.6,  lightColor: '#aabcc4', ambientIntensity: 0.2, isRaining: true },
  Cotton: { skyColor: '#ffffff', fogColor: '#f8f9fa', fogDensity: 0.04, lightIntensity: 1.2,  lightColor: '#ffffff', ambientIntensity: 0.5 },
};
...
const CAMPS = [
  { id: 'alpha',   name: 'Camp Alpha',   lat: 40.7128, lng: -74.0060,  description: 'Estructura de mando central. Blindaje nivel 5.', color: '#ff4444' },
  { id: 'bravo',   name: 'Camp Bravo',   lat: 34.0522, lng: -118.2437, description: 'Refugio de suministros médicos y biológicos.', color: '#44ff44' },
  { id: 'charlie', name: 'Camp Charlie', lat: 51.5074, lng: -0.1278,   description: 'Unidad de investigación de energía solar.', color: '#4444ff' },
  { id: 'delta',   name: 'Camp Delta',   lat: -33.8688, lng: 151.2093, description: 'Centro de comunicaciones transoceánicas.', color: '#ffff44' },
];
    ...
    <div className="w-full h-full relative bg-[#0a0805]">
      ...
      <div className="absolute top-12 left-12 z-10 max-w-md pointer-events-none">
        ...
        >
          ...
          <h2 className="text-6xl font-black text-white uppercase italic tracking-tighter leading-none" style={{ fontFamily: "'Oswald', sans-serif" }}>
            {camp.name}
          </h2>
...
export function MainHomePage() {
  ...
  const isAnyMenuOpen = isSettingsModalOpen || isModePanelOpen || showCredits || isUserGuideOpen || appState === 'menu' || appState === 'landing' || appState === 'login' || appState === 'register' || appState === 'global-map';
  ...
  useEffect(() => {
    if (appState === 'login' || appState === 'register' || appState === 'video' || appState === 'bridge' || appState === 'menu' || appState === 'intro' || appState === 'landing') {
  ...
  useEffect(() => {
    ...
    if (sunRef.current) {
      ((sunRef.current as THREE.Sprite).material as THREE.SpriteMaterial).color.set(config.lightColor);
    ...
    if (sunGlowRef.current) {
      ...
      sunGlowRef.current.intensity = (currentMode === 'Noche' || currentMode === 'Storm') ? 0 : config.lightIntensity * 10;
    }
  ...
  useEffect(() => {
    ...
    const terrainMesh = new THREE.Mesh(terrainGeo, new THREE.MeshStandardMaterial({ color: 0x111111, roughness: 0.9, metalness: 0.1 }));
    ...
    for (let i = 0; i < 150; i++) {
      ...
      if (!isDead) {
        ...
        for (let j = 0; j < simplifiedLayers; j++) {
          ...
          const layerHeight = 3;
          const leaf = new THREE.Mesh(new THREE.ConeGeometry(layerWidth, layerHeight, 6, 1, true), leavesMat);
    ...
    }, undefined, (e) => console.error("Sign Load Error:", e));

    gltfLoader.load(lightPoleUrl, (gltf) => {
    ...
    gltfLoader.load(areaSignUrl, (gltf) => {
        areaPositions.forEach(pos => {
    ...
    for (let i = 0; i < 40; i++) {
      ...
      bird.position.set((Math.random() - 0.5) * 600, 30 + Math.random() * 40, (Math.random() - 0.5) * 600);
      bird.userData = {
            ...
            {(appState === 'explore' || appState === 'menu') && (
              <div className="flex items-center gap-3 pointer-events-auto">
                <button
                  ...
                  title="Viento"
                >
                  <Wind size={28} className="transition-all group-hover:scale-110 anim-atmosphere-pulse" />
                </button>
              </div>
            )}
          </motion.nav>
                    ...
                    {(Object.keys(MODES) as Mode[]).map(mode => (
                      ...
                      >
                        <div className="relative z-10 w-full flex items-center gap-4">
                          <div className={`w-3 h-3 rounded-full border border-white/20 flex items-center justify-center transition-all ${currentMode === mode ? 'bg-white/10' : ''}`}>
                            {currentMode === mode && (
            ...
            <div className="relative group cursor-pointer pointer-events-auto anim-radio-shiver" onClick={() => setAppState('global-map')}>
              ...
              >
                <svg width="50" height="90" viewBox="0 0 50 90" fill="none" xmlns="http://www.w3.org/2000/svg">
                  ...
                  <g opacity="0.4">
                    {[30, 33, 36, 39].map(y => (
                      <line key={y} x1="14" y1={y} x2="34" y2={y} stroke="#334155" strokeWidth="1" strokeLinecap="round"/>
                    ))}
                  </g>
                  ...
                  {[
                    ...
                    <g key={i}>
                      ...
                      <line x1={pos.x - 0.8} y1={pos.y - 0.8} x2={pos.x + 0.8} y2={pos.y + 0.8} stroke="#222" strokeWidth="0.5" />
                    </g>
                  ))}
        ...
        {appState === 'explore' && !isAnyMenuOpen && (
          ...
          >
            ...
            <div className="relative z-10 flex flex-col gap-2">
              ...
              <div>
                <div className="text-[8px] uppercase tracking-[0.3em] text-white/40 mb-1 font-mono">Coordenadas</div>
              ...
              <div className="w-full md:w-[340px] h-full bg-black/60 backdrop-blur-2xl border-t md:border-t-0 md:border-r border-white/5 p-8 md:p-10 flex flex-col justify-center relative z-10 overflow-y-auto">
                ...
                <div className="relative z-10 w-full max-w-sm mx-auto">
                  ...
                  <form className="space-y-6" onSubmit={handleAuthSubmit}>
                    ...
                    <div className="space-y-3 w-full max-w-[260px]">
                      <label className="text-[11px] uppercase font-bold tracking-[0.2em] text-white block">Contraseña</label>
                      <input 
                        type="password" 
                        className="w-full bg-white/5 border border-white/20 rounded-xl px-4 py-2.5 text-white focus:outline-none focus:ring-1 focus:ring-blue-400/50 focus:border-blue-400/50 transition-all font-mono text-xs placeholder:text-white/10"
                        placeholder="••••••••"
                    ...
                    {appState === 'register' && (
                      <div className="space-y-3 w-full max-w-[260px]">
                        <label className="text-[11px] uppercase font-bold tracking-[0.2em] text-white block">Confirmar Clave</label>
                        <input 
                          type="password" 
                          className="w-full bg-white/5 border border-white/20 rounded-xl px-4 py-2.5 text-white focus:outline-none focus:ring-1 focus:ring-blue-400/50 focus:border-blue-400/50 transition-all font-mono text-xs placeholder:text-white/10"
                          placeholder="••••••••"
                  ...
                  <div className="mt-12 pt-10 border-t border-white/5">
                    {appState === 'login' ? (
                      <div className="flex items-center gap-4 text-[11px] uppercase font-bold tracking-widest text-white leading-relaxed">
                        <Compass size={32} className="shrink-0 text-blue-400 drop-shadow-[0_0_8px_rgba(59,130,246,0.5)]" />
                        <span>¿Sos nuevo? Explorá el mapa y solicitá ingreso a un campamento</span>
                      </div>
                    ) : (
                      <button 
                        onClick={() => { setAppState('login'); setCurrentMode('Storm'); }}
                    ...
                    <div className="mb-4 flex flex-col items-end">
                      ...
                      >
                        <span className="relative z-10">
                          {selectedCharacter === loginVideoIndex ? '✓ ELEGIDO' : 'SELECCIONAR PERSONAJE'}
                        </span>
                        {selectedCharacter !== loginVideoIndex && (
                ...
                {appState === 'register' && (
                  <div className="absolute bottom-6 left-0 right-0 z-40 flex flex-col items-center gap-2">
                    <div className="flex items-center justify-center gap-8">
                      <button 
                        onClick={() => setLoginVideoIndex(prev => (prev - 1 + loginVideos.length) % loginVideos.length)}
                      ...
                      <button 
                        onClick={() => setLoginVideoIndex(prev => (prev + 1) % loginVideos.length)}
                        className="p-2 text-white hover:text-white transition-colors"
                      >
              ...
              <div className="space-y-8">
                ...
                <div className="space-y-6">
                  <div className="bg-white/5 border border-white/10 p-5 rounded-lg">
                    <h3 className="text-[10px] font-black uppercase text-blue-400 tracking-widest mb-2 font-mono">Controles de Vuelo</h3>
                    <p className="text-sm text-white/80 leading-relaxed font-mono">
                      <span className="text-white font-bold">PC:</span> Mantén el click izquierdo presionado y mueve el ratón para orientar la cámara y explorar el entorno.<br/>
                      <span className="text-white font-bold">MÓVIL:</span> Toca la pantalla y desliza el dedo para girar la vista.
                    </p>
                  </div>

                  <div className="bg-white/5 border border-white/10 p-5 rounded-lg">
                    <h3 className="text-[10px] font-black uppercase text-blue-400 tracking-widest mb-2 font-mono">Redes del Campamento</h3>
                    <p className="text-sm text-white/80 leading-relaxed font-mono">
                      Localiza el ícono del <span className="text-blue-400 font-bold">Walkie-Talkie</span> en la interfaz para acceder a las redes de comunicación. Desde allí podrás conectar con otros refugios y visualizar el estado de la resistencia.
                    </p>
                  </div>
                </div>
              </div>
                ...
                {}
                  ...
                  <div className="flex flex-col gap-4">
                    ...
                    >
                      <div className="relative z-10 w-full flex items-center gap-4">
                        <div className="w-5 flex justify-center">
                          <Play size={14} className="fill-white group-hover/item:fill-blue-400 group-hover/item:text-blue-400 transition-colors" />
                        </div>
                        <span className="group-hover/item:text-blue-400 transition-colors">REANUDAR</span>
                      </div>
                    </button>
                    ...
                    >
                      ...
                      >
                        {volume === 0 ? (
                          <VolumeX size={24} className="text-white/80 group-hover/volbtn:text-white transition-colors drop-shadow-[0_0_4px_rgba(255,255,255,0.2)]" />
                        ) : (
                          <Volume2 size={24} className="text-white drop-shadow-[0_0_8px_rgba(255,255,255,0.3)]" />
                        )}
                      </button>
                    ...
                    >
                      <div className="relative z-10 w-full flex items-center gap-4">
                        ...
                        <span className="group-hover/item:text-blue-400 transition-colors">LOBBY</span>
                      </div>
                    </button>
                    ...
                    >
                      <div className="relative z-10 w-full flex items-center gap-4">
                        ...
                        <span className="group-hover/item:text-blue-400 transition-colors">VOLVER AL INICIO</span>
                      </div>
                    </button>
                    ...
                    >
                      <div className="relative z-10 w-full flex items-center gap-4">
                        ...
                        <span className="group-hover/item:text-blue-400 transition-colors">SALIR</span>
                      </div>
                    </button>
                  </div>
              </div>
