...
import { LoadingScreen } from "../../expeditions-ui/components/LoadingScreen";
import { OrientationWarning } from "../../expeditions-ui/components/OrientationWarning";
...
const NAVIGATION_DATA: NavItem[] = [
  ...
  { id: "inventario", label: "Inventario", icon: <CrateIcon />, subOptions: ["Resumen", "Ítems", "Movimientos", "Alertas", "Recolección"] },
  {
    ...
    label: "Expediciones",
    icon: <CompassIcon />,
    subOptions: ["Activas", "Planificación", "Historial", "Mapa", "Recursos"],
  },
...
export default function AdminDashboardUiV2Page() {
  ...
  const [activeNav, setActiveNav] = useState<AdminSectionId>("poblacion");
  const [activeSub, setActiveSub] = useState<string>(NAVIGATION_DATA.find((item) => item.id === "poblacion")?.subOptions[0] ?? "Estadísticas");
  ...
  const loadCoreData = async () => {
    ...
    setDataError(null);
    try {
      const [personsData, campsData, occupationsData, admissionsData, expeditionsData] = await Promise.all([
        fetchPersons(),
        fetchCamps(),
        fetchOccupations(),
        listAdmissionRequests(),
  ...
  return (
    <div className="game-screen-layout text-[#A4C2C5]">
      ...
      {hasEntered && <TopHud onBack={() => navigate("/app")} onLogout={handleLogout} />}
...
function TopHud({ onBack, onLogout }: { onBack: () => void; onLogout: () => void }) {
  return (
    ...
    <div className="admin-ui-v2-content">
      ...
      {section === "centro" && (
        <div className="admin-ui-v2-grid admin-ui-v2-grid-4">
          <MetricCard label="Supervivientes" value={populationStats.total} tone="info" />
          <MetricCard label="Admisiones Pendientes" value={admissionsQueue.length} tone="warn" />
          <MetricCard label="Expediciones Activas" value={expeditions.filter((item) => item.status !== "COMPLETADA").length} tone="info" />
          <MetricCard label="Solicitudes Intercamp" value={intercampRequests.length} tone="ok" />
        </div>
      )}
...
function AdmissionsModule({
  ...
  useEffect(() => {
    setPage(1);
  }, [sub, admissionsQueue.length, admissionsHistory.length]);
...
function ExpeditionsModule({
  ...
  const activeExpeditions = expeditions.filter((item) => item.status !== "COMPLETADA");
  const plannedExpeditions = expeditions.filter((item) => item.status === "PROGRAMADA");
  const historyExpeditions = expeditions.filter((item) => item.status === "COMPLETADA");
  const returningExpeditions = expeditions.filter((item) => item.status === "REGRESANDO");

  const visibleExpeditions = useMemo(() => {
    if (sub === "Activas") return activeExpeditions;
    if (sub === "Planificación") return plannedExpeditions;
    if (sub === "Historial") return historyExpeditions;
    return expeditions;
  }, [activeExpeditions, expeditions, historyExpeditions, plannedExpeditions, sub]);
  ...
  return (
    <div className="admin-ui-v2-expeditions">
      <div className="admin-ui-v2-grid admin-ui-v2-grid-4">
        <MetricCard label="En curso" value={expeditions.filter((item) => item.status === "EN CURSO").length} tone="info" />
      ...
      {(sub === "Activas" || sub === "Planificación" || sub === "Historial") && (
          ...
          <div className="admin-ui-v2-module-card">
            {selectedExpedition ? (
              <div className="admin-ui-v2-expedition-detail">
                ...
                <div><strong>Estado:</strong> {selectedExpedition.status}</div>
                <div><strong>Sector:</strong> {selectedExpedition.sector}</div>
                <div><strong>Progreso:</strong> Dia {selectedExpedition.day} de {selectedExpedition.total}</div>
                <div><strong>Participantes:</strong> {selectedExpedition.participants.length}</div>
