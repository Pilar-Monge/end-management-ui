...
import { FormState, CalendarDay } from '../types';
...
const healthFields = [
  ...
  { name: 'habilidades', label: 'Habilidades', placeholder: 'Navegación GPS, primeros auxilios, cocina' },
] as const;
...
const fieldsetStyle = {
  border: '2px solid rgba(15, 12, 8, 0.95)',
  background: 'rgba(248, 232, 195, 0.62)',
  boxShadow: 'inset 0 0 0 1px rgba(91, 61, 35, 0.2)',
  padding: '16px',
};
...
const legendStyle = {
  ...
  border: '1.5px solid rgba(15, 12, 8, 0.5)',
};
...
export default function AdmissionPage() {
  ...
  return (
    <main style={{ position: 'relative', minHeight: '100vh', overflow: 'hidden', background: '#15120d', padding: '20px 16px', color: '#1a1410' }}>
      <div style={{ position: 'absolute', inset: 0, background: 'radial-gradient(circle at 20% 15%, rgba(139,35,29,.4), transparent 30%), radial-gradient(circle at 78% 6%, rgba(233,211,160,.18), transparent 24%), linear-gradient(115deg, rgba(0,0,0,.48), transparent 42%, rgba(0,0,0,.62))' }} />
      <div className="ash ash-one" />
      <div className="ash ash-two" />
      <div className="ember ember-one" />
      <div className="ember ember-two" />

      <section className="dossier" style={{ position: 'relative', maxWidth: '1180px', margin: '0 auto', padding: '20px', boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.25)', zIndex: 1 }}>
        ...
        <div style={{ marginBottom: '8px', display: 'flex', justifyContent: 'flex-end', paddingRight: '16px' }}>
          <div className="stamp confidential-stamp" style={{ display: 'inline-block', transform: 'rotate(3deg)', padding: '6px 20px', fontWeight: 900, textTransform: 'uppercase', letterSpacing: '0.28em' }}>
            Confidencial
          </div>
        </div>

        <header style={{ marginBottom: '20px', display: 'flex' }}>
          <div className="header-card" style={{ position: 'relative', display: 'inline-block', paddingBottom: '16px' }}>
            <p className="header-eyebrow" style={{ fontFamily: "ui-monospace, 'Courier New', monospace", fontSize: '0.82rem', fontWeight: 900, letterSpacing: '0.36em', textTransform: 'uppercase', color: '#1a1410', margin: 0 }}>Registro de sobreviviente</p>
            <h1 className="stencil" style={{ marginTop: '4px', fontSize: 'clamp(2.5rem, 8vw, 4rem)', fontWeight: 900, textTransform: 'uppercase', lineHeight: 1, letterSpacing: '0.05em', color: '#0a0806' }}>Camp Zero</h1>
            <p className="header-subtitle" style={{ marginTop: '8px', fontFamily: "ui-monospace, 'Courier New', monospace", fontSize: '1rem', fontWeight: 900, letterSpacing: '0.05em', textTransform: 'uppercase', color: '#6b1414' }}>Formulario de ingreso a zona segura</p>
          </div>
        </header>

        <form onSubmit={handleSubmit} style={{ position: 'relative', display: 'grid', gap: '20px', gridTemplateColumns: window.innerWidth >= 1024 ? '1fr 305px' : '1fr' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            <fieldset className="file-section animate-rise" style={fieldsetStyle}>
              <legend className="section-title" style={legendStyle}>Datos personales</legend>
              <div style={{ display: 'grid', gap: '16px', gridTemplateColumns: window.innerWidth >= 640 ? 'repeat(2, 1fr)' : '1fr' }}>
                {textFields.map((field) => (
                  <label key={field.name} className="field-label" style={labelStyle}>
                    {field.label}
                    <input className="classified-input" maxLength={field.maxLength} name={field.name} onChange={handleInput} required={field.required} value={form[field.name]} style={inputStyle as any} />
                  </label>
                ))}
                <label className="field-label" style={labelStyle}>
                  Email
                  <input className="classified-input" name="email" onChange={handleInput} placeholder="correo@ejemplo.com" required type="email" value={form.email} style={inputStyle as any} />
                </label>
                <label className="field-label" style={labelStyle}>
                  Usuario
                  <input className="classified-input" maxLength={30} minLength={3} name="usuario" onChange={handleInput} pattern="[A-Za-z0-9-]+" placeholder="sobreviviente-01" required value={form.usuario} style={inputStyle as any} />
                </label>
                <div className="field-label" style={labelStyle}>
                  Género
                  <button aria-expanded={genderOpen} className="custom-trigger" onClick={() => setGenderOpen((current) => !current)} type="button" style={{ alignItems: 'center', background: 'rgba(255, 245, 215, 0.55)', border: 0, borderBottom: '3px solid #0d0b08', color: '#06050a', cursor: 'pointer', display: 'flex', fontFamily: "ui-monospace, 'Courier New', monospace", fontSize: '1.02rem', fontWeight: 800, justifyContent: 'space-between', marginTop: '0.5rem', minHeight: '2.7rem', padding: '0.65rem 0.55rem 0.55rem', textAlign: 'left', width: '100%' } as any}>
                    <span>{form.genero || 'Seleccionar'}</span>
                    <span style={{ color: '#8f1d1d', fontSize: '1.1rem' }}>▾</span>
                  </button>
                </div>
                <div className="field-label" style={{ ...labelStyle, gridColumn: window.innerWidth >= 1024 ? 'span 2' : 'auto' } as any}>
                  Fecha de nacimiento
                  <button aria-expanded={calendarOpen} className="custom-trigger" onClick={() => setCalendarOpen((current) => !current)} type="button" style={{ alignItems: 'center', background: 'rgba(255, 245, 215, 0.55)', border: 0, borderBottom: '3px solid #0d0b08', color: '#06050a', cursor: 'pointer', display: 'flex', fontFamily: "ui-monospace, 'Courier New', monospace", fontSize: '1.02rem', fontWeight: 800, justifyContent: 'space-between', marginTop: '0.5rem', minHeight: '2.7rem', padding: '0.65rem 0.55rem 0.55rem', textAlign: 'left', width: '100%' } as any}>
                    <span>{formatDate(form.nacimiento)}</span>
                    <span className="calendar-icon" style={{ border: '2px solid #17140f', height: '18px', width: '18px', borderRadius: '1px', position: 'relative' }} />
                  </button>
                </div>
              </div>
            </fieldset>

            <fieldset className="file-section animate-rise-delay" style={fieldsetStyle}>
              <legend className="section-title" style={legendStyle}>Información de salud</legend>
              <div style={{ display: 'grid', gap: '16px', gridTemplateColumns: window.innerWidth >= 640 ? 'repeat(2, 1fr)' : '1fr' }}>
                {healthFields.map((field) => (
                  <label key={field.name} className="field-label" style={labelStyle}>
                    {field.label}
                    <textarea className="classified-input" name={field.name} onChange={handleInput} placeholder={field.placeholder} required value={form[field.name]} style={{ ...inputStyle, minHeight: '64px', resize: 'vertical' } as any} />
                  </label>
                ))}
              </div>
            </fieldset>
          ...
          <aside style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            <label className="photo-drop" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '20.8rem', cursor: 'pointer', overflow: 'hidden', border: '2px solid rgba(15, 12, 8, 0.95)', backgroundColor: '#f0dcae', backgroundImage: 'url("/images/paper-clean.jpg")', backgroundSize: 'cover', backgroundPosition: 'center', backgroundBlendMode: 'multiply', boxShadow: '0 10px 22px rgba(30, 18, 10, 0.42), inset 0 0 25px rgba(77, 45, 22, 0.18)', transform: 'rotate(0.5deg)', position: 'relative', color: '#06050a' } as any}>
              {photoPreview ? (
      ...
      {genderOpen && (
        <div className="modal-backdrop" onClick={() => setGenderOpen(false)} style={{ position: 'fixed', inset: 0, background: 'radial-gradient(circle at center, rgba(20, 12, 8, 0.55), rgba(8, 4, 2, 0.85))', backdropFilter: 'blur(3px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 80, padding: '1rem' } as any}>
          <div className="dropdown-panel" onClick={(event) => event.stopPropagation()} style={{ backgroundColor: '#f0dcae', backgroundImage: 'url("/images/paper-clean.jpg")', backgroundSize: 'cover', backgroundPosition: 'center', backgroundBlendMode: 'multiply', border: '2px solid rgba(15, 12, 8, 0.95)', boxShadow: '0 25px 55px rgba(0, 0, 0, 0.7), inset 0 0 25px rgba(77, 45, 22, 0.18)', color: '#06050a', position: 'relative', zIndex: 81, padding: '0.6rem', minWidth: '260px' } as any}>
      ...
      {calendarOpen && (
        <div className="modal-backdrop" onClick={() => setCalendarOpen(false)} style={{ position: 'fixed', inset: 0, background: 'radial-gradient(circle at center, rgba(20, 12, 8, 0.55), rgba(8, 4, 2, 0.85))', backdropFilter: 'blur(3px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 80, padding: '1rem' } as any}>
          <div className="calendar-popover" onClick={(event) => event.stopPropagation()} style={{ backgroundColor: '#f0dcae', backgroundImage: 'url("/images/paper-clean.jpg")', backgroundSize: 'cover', backgroundPosition: 'center', backgroundBlendMode: 'multiply', border: '2px solid rgba(15, 12, 8, 0.95)', boxShadow: '0 25px 55px rgba(0, 0, 0, 0.7), inset 0 0 25px rgba(77, 45, 22, 0.18)', color: '#06050a', position: 'relative', zIndex: 81, padding: '1rem', width: 'min(360px, 96vw)' } as any}>
            ...
            <div style={{ marginTop: '0.7rem', borderTop: '1.5px solid rgba(15, 12, 8, 0.45)', paddingTop: '0.55rem', display: 'flex', justifyContent: 'space-between', gap: '8px' }}>
              <button onClick={() => { updateFormValue('nacimiento', ''); setCalendarOpen(false); }} type="button" style={{ background: 'transparent', border: 0, color: '#5a1414', cursor: 'pointer', fontFamily: 'inherit', fontWeight: 900, textTransform: 'uppercase', letterSpacing: '0.12em', fontSize: '0.88rem', padding: '0.4rem 0.6rem' }}>Borrar</button>
              <button onClick={() => selectCalendarDay({ date: new Date(), inMonth: true, isSelected: false, isToday: true, value: toDateValue(new Date()) })} type="button" style={{ background: 'transparent', border: 0, color: '#5a1414', cursor: 'pointer', fontFamily: 'inherit', fontWeight: 900, textTransform: 'uppercase', letterSpacing: '0.12em', fontSize: '0.88rem', padding: '0.4rem 0.6rem' }}>Hoy</button>
            </div>
          </div>
        </div>
      )}
